...
export class BullMQService {
  ...
  private newProjectRegistrationQueue: Queue | null = null; // Queue for new project registrations
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Assumes newProjectRegistrationQueueName() will be available via redisKeys
      // e.g., `${this.options.prefix || ''}system:new-project-registration`
      // For consistency, the name should be generated by createRedisKeys
      const baseRegistrationQueueName = redisKeys.newProjectRegistrationQueueName(); // Should be 'system-new-project-registration'
      const bullmqPrefix = redisKeys.getBullMQPrefix() ?? ''; // Get prefix, default to ''
      
      this.newProjectRegistrationQueue = new Queue(baseRegistrationQueueName, {
        connection: this.redisManager.getConnection(),
        prefix: bullmqPrefix, // Use the calculated prefix
        // prefix: '', // The queue name already includes the prefix from getRedisKeys()
        defaultJobOptions: this.options.defaultJobOptions // Options for this queue can be configured
      });
      // Note: Consider adding this queue to Bull Board if monitoring is needed.

      // 加载计数器
      await this.loadCounters();
      
      // 初始化WorkerManager
      // WorkerManager must be initialized to listen to the new project registration queue.
      await this.workerManager.initialize(); 
      
      // Calling initAll() here might no longer be required for immediate registration of all existing project workers,
      // as new projects will be registered via the queue.
      // WorkerManager.initAll() logic should be adjusted accordingly.
      // await this.workerManager.initAll(); 
  ...
  public getProjectQueue(projectId: string): Queue {
    ...
    const redisKeys = this.getRedisKeys();
    const baseQueueName = redisKeys.projectQueueName(projectId); // e.g. 'project-projectID'
    const bullmqOptPrefix = redisKeys.getBullMQPrefix() ?? ''; // Get prefix, default to ''
    
    // Create the fully qualified map key that BullMQ uses internally
    const mapKey = bullmqOptPrefix ? `${bullmqOptPrefix}:${baseQueueName}` : baseQueueName;
    
    if (!this.queues.has(mapKey)) {
      try {
        console.log(`创建队列: ${mapKey} (项目ID: ${projectId})`);
        
        // 使用标准化的队列名创建队列
        const queue = new Queue(baseQueueName, { // Use base name here
          connection: this.redisManager.getConnection(),
          prefix: bullmqOptPrefix, // Use calculated prefix here
          // prefix: '', // OLD: Completely disable any prefix
          ...this.options.projectQueueOptions
        });
        this.queues.set(mapKey, queue); // Store with fully qualified key
        
        // 当创建新队列时，自动添加到 Bull Board
        try {
          // 使用异步调用但不等待结果，以避免阻塞
          // 传递当前前缀以确保Bull Board使用相同的队列名称
          // TODO: Review addQueueToBoard - it needs the projectId and the prefix, maybe mapKey is better?
          // For now, stick to original params, but this might need adjusting if Bull Board needs the qualified name.
          addQueueToBoard(projectId, this.getCurrentPrefix()).catch(error => {
            // 只记录错误但不影响队列创建
            console.warn(`将队列 ${mapKey} 添加到 Bull Board 失败:`, error);
          });
        } catch (error) {
          // 忽略错误，即使 Bull Board 添加失败也不影响队列正常工作
          console.warn(`尝试将队列 ${mapKey} 添加到 Bull Board 时出错:`, error);
        }
      } catch (error) {
        console.error(`创建队列 ${mapKey} 时出错:`, error);
        throw error;
      }
    }
    
    return this.queues.get(mapKey)!;
  }
  ...
  public async createProject(
    ...
    try {
      ...
      // 确保创建项目队列 (for project tasks)
      this.getProjectQueue(projectId);
      
      // 为新项目注册Worker - now via queue
      // Remove direct call: await this.workerManager.register(projectId);
      if (!this.newProjectRegistrationQueue) {
        // This shouldn't happen if initialize() completed successfully
        console.error('New project registration queue is not initialized.');
        throw new AppError('New project registration queue is not ready', AppErrorCode.ServiceNotReadyError);
      }
      
      // Добавляем задачу в очередь регистрации проектов
      await this.newProjectRegistrationQueue.add('registerProject', { 
        projectId, 
        tenantId: projectData.tenantId // Pass tenantId if it exists
      });
  ...
  public async close(): Promise<void> {
    ...
    try {
      ...
      // 关闭所有队列
      for (const [mapKey, queue] of this.queues.entries()) {
        await queue.close();
      }

      // Close the new project registration queue if it exists
      if (this.newProjectRegistrationQueue) {
        await this.newProjectRegistrationQueue.close();
      }
  ...
  public setPrefix(prefix?: string): void {
    ...
    if (this.serviceState === BullMQServiceState.READY) {
      ...
      // Close and recreate the new project registration queue if it exists
      // and if the prefix change should affect it (system queues usually don't change prefixes often)
      // In this case, assume getRedisKeys() will account for the new prefix for newProjectRegistrationQueueName()
      if (this.newProjectRegistrationQueue) {
        this.newProjectRegistrationQueue.close().catch(err => {
            console.error('Error closing the new project registration queue during prefix change:', err);
        });
        const newRedisKeys = this.getRedisKeys();
        const baseRegistrationQueueName = newRedisKeys.newProjectRegistrationQueueName();
        const newBullmqPrefix = newRedisKeys.getBullMQPrefix() ?? '';
        this.newProjectRegistrationQueue = new Queue(baseRegistrationQueueName, {
          connection: this.redisManager.getConnection(),
          prefix: newBullmqPrefix, // Use new prefix
          defaultJobOptions: this.options.defaultJobOptions // Options for this queue can be configured
        });
      }
